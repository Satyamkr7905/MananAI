...
const ICONS = {
  hardest:     { icon: Trophy,  tone: "bg-amber-50 text-amber-600" },
  achievement: { icon: Sparkles, tone: "bg-brand-50 text-brand-600" },
  levelup:     { icon: Award,    tone: "bg-emerald-50 text-emerald-600" },
};
...
export default function Highlight({ item }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      <div className="text-xs text-slate-400 shrink-0">{relativeTime(item.when)}</div>
    </div>
  );
}
